...
/// </summary>
public sealed class ResamplingService
{
    #region Properties
    ...
    public Bitmap Resample(Bitmap input, int nWidth, int nHeight)
    {
        
        Span<byte> arr = stackalloc byte[4 * input.Width * input.Height];
            ...
            BitmapData originalBitmapData = input.LockBits(new Rectangle(0, 0, input.Width, input.Height), ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
            int nOffsetOriginal = (originalBitmapData.Stride - originalBitmapData.Width * 4);

            unsafe
            {

                byte* p = (byte*)originalBitmapData.Scan0;

                for (int y = 0; y < originalBitmapData.Height; y++)
                {
                    for (int x = 0; x < originalBitmapData.Width; x++)
                    {
                        int index = y * originalBitmapData.Width + x;
                        arr[index + 3] = p[3];
                        arr[index + 2] = p[2];
                        arr[index + 1] = p[1];
                        arr[index + 0] = p[0];
            ...
            input.UnlockBits(originalBitmapData);

        ResamplingFilter filter = ResamplingFilter.Create(this.filter);

        int width = originalBitmapData.Width;
        int height = originalBitmapData.Height;
        int planes = arr.Length;

        this.covered = 0;
        this.total = (nWidth + height);

        // create bitmaps
        Span<byte> workSpan = stackalloc byte[4 * width * height];
        Span<byte> outputSpan = stackalloc byte[4 * nWidth * nHeight];
        /*ushort[][,] work = new ushort[planes][,];
        ushort[][,] output = new ushort[planes][,];*/
        /*int c = 0;
            ...
            for (k = 0; k < height; k++)
            {
                for (i = 0; i < nWidth; i++)
                {
                    ...
                    for (j = 0; j < contrib[i].n; j++)
                    {
                        ...
                        intensity += (/*input[c][contrib[i].p[j].pixel, k] */arr[c + k + contrib[i].p[j].pixel* height] * weight);
                    }

                    workSpan[c + i* width + k] /*work[c][i, k]*/ = (byte)Math.Min(Math.Max(intensity / contrib[i].wsum, UInt16.MinValue), UInt16.MaxValue);
                }
            ...
            for (k = 0; k < nWidth; k++)
            {
                for (i = 0; i < nHeight; i++)
                {
                    ...
                    for (j = 0; j < contrib[i].n; j++)
                    {
                        ...
                        intensity += /*(work[c][k, contrib[i].p[j].pixel] * weight) */workSpan[c + contrib[i].p[j].pixel + k* width] * weight;
                    }

                    /*output[c][k, i]*/ outputSpan[c + k *nWidth + i] = (byte)Math.Min(Math.Max(intensity, UInt16.MinValue), UInt16.MaxValue);
                }
        ...
        unsafe
        {
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    p[3] = (byte)Math.Min(Math.Max(outputSpan[index + 3] /*array[3][x, y]*/, Byte.MinValue), Byte.MaxValue);
                    p[2] = (byte)Math.Min(Math.Max(outputSpan[index + 2] /*array[2][x, y]*/, Byte.MinValue), Byte.MaxValue);
                    p[1] = (byte)Math.Min(Math.Max(outputSpan[index + 1] /*array[1][x, y]*/, Byte.MinValue), Byte.MaxValue);
                    p[0] = (byte)Math.Min(Math.Max(outputSpan[index + 0] /*array[0][x, y]*/, Byte.MinValue), Byte.MaxValue);
...
#endregion ResamplingService class

#region ResamplingFilters enum

public enum ResamplingFilters:byte
{
    Box = 0,
    Triangle,
    Hermite,
...
#endregion ResamplingFilters enum
...
internal sealed class HermiteFilter : ResamplingFilter
{
...
internal sealed class BoxFilter : ResamplingFilter
{
    public BoxFilter()
    {
...
internal sealed  class TriangleFilter : ResamplingFilter
{
...
internal sealed class BellFilter : ResamplingFilter
{
    public BellFilter()
    {
...
internal sealed class CubicBSplineFilter : ResamplingFilter
{
    private double temp;
...
internal sealed class Lanczos3Filter : ResamplingFilter
{
...
internal sealed class MitchellFilter : ResamplingFilter
{
...
internal sealed class CosineFilter : ResamplingFilter
{
    public CosineFilter()
    {
        defaultFilterRadius = 1;
...
internal sealed class CatmullRomFilter : ResamplingFilter
{
...
internal sealed class QuadraticFilter : ResamplingFilter
{
...
internal sealed class QuadraticBSplineFilter : ResamplingFilter
{
...
internal sealed class CubicConvolutionFilter : ResamplingFilter
{
    private double temp;
...
internal sealed class Lanczos8Filter : ResamplingFilter
{
